...
export default async function SiteFooter() {
  ...
  const socialLinks: { url: string; label: string; icon: React.JSX.Element; hover: string }[] = [];
  if (settings.social_facebook) socialLinks.push({
  ...
  if (settings.social_instagram) socialLinks.push({
  ...
  if (settings.social_twitter) socialLinks.push({
  ...
  if (settings.social_tiktok) socialLinks.push({
  ...
  if (settings.social_youtube) socialLinks.push({
  ...
  const storeEmail = settings.store_email || site.contactEmail;
  const storeWhatsapp = settings.store_whatsapp || site.whatsapp;
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
          <div className="max-w-sm">
            <div className="flex items-center gap-2.5 text-lg font-extrabold">
              <Image src="/logo.png" alt={`شعار ${storeName}`} width={36} height={36} className="h-9 w-9 rounded-lg object-contain" />
          ...
          <div className="grid grid-cols-2 gap-8 text-sm">
              ...
              <ul className="space-y-2 text-muted">
                {footerLinks.length > 0
                  ? footerLinks.map((link) => (
                      <li key={link.url}>
                        <Link href={link.url} className="hover:text-brand-300">
                          {link.label}
                        </Link>
                      </li>
                    ))
            ...
            <div>
              ...
              <ul className="space-y-2 text-muted">
                <li>
                  <a href={`mailto:${storeEmail}`} className="hover:text-brand-300">
                    {storeEmail}
                  </a>
                </li>
                <li>
                  <a href={`https://wa.me/${storeWhatsapp}`} className="hover:text-brand-300">
                    واتساب
                  </a>
                </li>
              </ul>
          ...
          <div className="mt-8 flex items-center gap-3">
            {socialLinks.map((s) => (
              <a
                ...
                className={`grid h-9 w-9 place-items-center rounded-lg border border-line bg-surface text-muted transition-colors ${s.hover}`}
                aria-label={s.label}
              >
                {s.icon}
              </a>
