...
<body>
  <div class="header">
    ...
    <div class="order-info">
      ...
      ${data.paymentMethodFields && data.paymentMethodFields.length > 0 ? data.paymentMethodFields.map(f => `<p style="margin:2px 0;font-size:13px;color:#6b7280;">${f.label}: <strong style="color:#1a1a2e;">${f.value}</strong></p>`).join("") : ""}
    </div>
  </div>
  ...
  return (
    <button
      ...
      onClick={handleOpen}
      disabled={loading}
      className="inline-flex items-center gap-2 rounded-xl border border-line bg-surface px-4 py-2.5 text-sm font-semibold text-fg transition-colors hover:bg-surface-2 disabled:opacity-60"
    >
